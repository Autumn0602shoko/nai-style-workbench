...
const fetchDanbooruResponse = async (url, timeoutMs = 18_000, retry = true) => {
  try {
    const response = await fetch(url, { headers: { "User-Agent": "NAI-Style-Workbench/0.12.0" }, signal: AbortSignal.timeout(timeoutMs) });
...
app.whenReady().then(() => {
  ...
  protocol.handle("nai-image", async (request) => {
    try {
      ...
      return net.fetch(imageUrl.toString(), { headers: { "User-Agent": "NAI-Style-Workbench/0.12.0" } });
