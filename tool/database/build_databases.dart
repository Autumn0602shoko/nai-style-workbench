...
// 批次大小
// Keep each multi-row INSERT below SQLite's bound-variable limit.
...
Future<void> buildTranslationDatabase() async {
  ...
  final outputPath = p.absolute(p.join(outputDir.path, 'translation.db'));
...
Future<void> buildCooccurrenceDatabase() async {
  ...
  final outputDir = Directory('assets/databases');
  final outputPath = p.absolute(p.join(outputDir.path, 'cooccurrence.db'));
